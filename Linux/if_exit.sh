...
# If x is greater than y, run the echo command - only works for 
integer values
if [ $x -gt $y ]
then
...
# check if $str1 is equal to 'this string' AND $x is greater than 
$y
...
then
  ...
  echo "Either those strings don't match, or $x is not greater 
than $y"
fi
...
# if the uid of the user running this script does not equal 1000, 
